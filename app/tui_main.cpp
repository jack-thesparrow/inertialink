// tui_main.cpp — Inertialink Terminal UI
//
// A lazygit-style launcher that lets you select a connection mode and
// start/stop any project tool (visualizer, decoder, training, etc.)
// from a single terminal window.  stdout+stderr from every spawned
// process is captured and shown live in the bottom log panel.
//
// Layout:
//   Left  : Connection (RadioBox) + per-process status dots
//   Right : Action list (run/stop) + Mock-ESP32 word input + live log
//   Bottom: Key-binding bar
//
// Key bindings:
//   Tab          switch panel  (Connection → Actions → Mock-word input)
//   ↑ / ↓        navigate action list
//   Enter        launch or stop the selected action
//   K            kill every running process
//   C            clear the log
//   PgUp/PgDn    scroll the log
//   Q            quit (stops all processes cleanly)

#include "pen/io.hpp"

#include <ftxui/component/component.hpp>
#include <ftxui/component/screen_interactive.hpp>
#include <ftxui/dom/elements.hpp>

#include <atomic>
#include <chrono>
#include <cstring>
#include <ctime>
#include <deque>
#include <fcntl.h>
#include <mutex>
#include <sstream>
#include <string>
#include <sys/wait.h>
#include <thread>
#include <unistd.h>
#include <vector>

using namespace ftxui;

// ── Shared log ───────────────────────────────────────────────────────────────
struct LogLine { std::string ts, src, msg; };

static std::mutex           gLogMtx;
static std::deque<LogLine>  gLog;
static constexpr int        LOG_CAP = 500;

static std::string nowTs() {
  auto t = std::time(nullptr);
  struct tm tm{};
  localtime_r(&t, &tm);
  char buf[9];
  std::strftime(buf, sizeof(buf), "%H:%M:%S", &tm);
  return buf;
}

static void logPush(const std::string &src, const std::string &msg) {
  std::lock_guard lk(gLogMtx);
  gLog.push_back({nowTs(), src, msg});
  if (static_cast<int>(gLog.size()) > LOG_CAP)
    gLog.pop_front();
}

// ── Process management ────────────────────────────────────────────────────────
struct Proc {
  std::string name;
  pid_t pid   = -1;
  int   rfd   = -1;   // read-end of stdout+stderr pipe
  bool  alive = false;
};

static Proc procLaunch(const std::string &name, std::vector<std::string> args) {
  int fds[2];
  if (::pipe(fds) < 0) { logPush("tui", "pipe() failed"); return {}; }

  pid_t pid = ::fork();
  if (pid < 0) {
    ::close(fds[0]); ::close(fds[1]);
    logPush("tui", "fork() failed"); return {};
  }
  if (pid == 0) {                            // child
    ::dup2(fds[1], STDOUT_FILENO);
    ::dup2(fds[1], STDERR_FILENO);
    ::close(fds[0]); ::close(fds[1]);
    std::vector<char *> argv;
    for (auto &a : args) argv.push_back(a.data());
    argv.push_back(nullptr);
    ::execvp(argv[0], argv.data());
    ::_exit(127);
  }
  ::close(fds[1]);
  ::fcntl(fds[0], F_SETFL, O_NONBLOCK);
  logPush("tui", "Started " + name + " (pid " + std::to_string(pid) + ")");
  return {name, pid, fds[0], true};
}

static void procStop(Proc &p) {
  if (!p.alive) return;
  if (p.pid > 0)  ::kill(p.pid, SIGTERM);
  if (p.pid > 0)  ::waitpid(p.pid, nullptr, WNOHANG);
  if (p.rfd >= 0) ::close(p.rfd);
  logPush("tui", "Stopped " + p.name);
  p = {};
}

// Drain all process pipes into the log.  Returns true if anything was read.
static bool procDrainAll(std::vector<Proc> &procs) {
  bool any = false;
  char buf[1024];
  for (auto &p : procs) {
    if (p.rfd < 0) continue;
    std::string chunk;
    ssize_t n;
    while ((n = ::read(p.rfd, buf, sizeof(buf) - 1)) > 0) {
      buf[n] = '\0'; chunk += buf; any = true;
    }
    std::istringstream ss(chunk);
    for (std::string line; std::getline(ss, line);)
      if (!line.empty()) logPush(p.name, line);

    if (p.pid > 0) {
      int st;
      if (::waitpid(p.pid, &st, WNOHANG) > 0) {
        logPush("tui", p.name + " exited (code "
                + std::to_string(WEXITSTATUS(st)) + ")");
        ::close(p.rfd);
        p.rfd = -1; p.pid = -1; p.alive = false;
      }
    }
  }
  return any;
}

// ── Main ──────────────────────────────────────────────────────────────────────
int main() {
  auto screen = ScreenInteractive::Fullscreen();

  // ── Connection ─────────────────────────────────────────────────────────────
  int conn = 0;
  const std::vector<std::string> kConnLabels = {
      "WiFi        (port " + std::to_string(pen::Defaults::wifiPort)
                           + " / " + std::to_string(pen::Defaults::wifiVizPort) + ")",
      std::string("USB         (") + pen::Defaults::usbPort       + ")",
      std::string("Bluetooth   (") + pen::Defaults::bluetoothPort + ")",
      "Simulation  (/tmp/vtty_laptop)",
  };
  const std::vector<std::string> kConnArgs = {"wifi", "usb", "bt", "sim"};

  // ── Action definitions ─────────────────────────────────────────────────────
  struct ActionDef {
    std::string name;
    std::string hint;
    std::vector<std::string> cmd;
    bool useConnArg;  // append wifi / usb / bt / sim
  };

  const std::vector<ActionDef> kActions = {
    { "Visualizer", "3D cube + stroke canvas",
      {"./bin/visualizer"},                          true  },
    { "Decoder",    "Real-time AI recognition",
      {"./bin/decoder"},                             true  },
    { "Collector",  "Record IMU data to CSV",
      {"./bin/data_collector"},                      true  },
    { "Mock ESP32", "",          // hint shows word dynamically
      {".venv/bin/python3", "scripts/mock_esp32.py"}, false },
    { "Train",      "BiLSTM CTC model training",
      {".venv/bin/python3", "scripts/train_bilstm.py"}, false },
    { "Evaluate",   "Batch accuracy on all samples",
      {".venv/bin/python3", "scripts/eval_model.py"}, false },
  };
  static constexpr int MOCK_IDX = 3;

  int act = 0;
  std::vector<Proc> procs(kActions.size());
  std::string mockWord = "hello";

  // Launch or stop the action at index i
  auto toggle = [&](int i) {
    if (procs[i].alive) { procStop(procs[i]); return; }
    auto cmd = kActions[i].cmd;
    if (kActions[i].useConnArg) cmd.push_back(kConnArgs[conn]);
    if (i == MOCK_IDX) cmd.push_back(mockWord.empty() ? "hello" : mockWord);
    procs[i] = procLaunch(kActions[i].name, cmd);
  };

  // ── FTXUI components ───────────────────────────────────────────────────────
  auto connRadio = Radiobox(&kConnLabels, &conn);
  auto mockInput = Input(&mockWord, "word / 'all'");

  // focus: 0=connection, 1=actions, 2=mock-word input
  int focus      = 1;
  int ftFocus    = 0;  // index into Container::Tab children
  int logScroll  = 0;  // lines scrolled up from bottom (0 = latest)

  // Map our 3-panel focus to the two real FTXUI components
  auto syncFtFocus = [&] {
    if      (focus == 0) ftFocus = 0;  // connRadio active
    else if (focus == 2) ftFocus = 1;  // mockInput active
    // focus==1 (actions): neither real component is active; handled by CatchEvent
  };
  syncFtFocus();

  auto layout = Container::Tab({connRadio, mockInput}, &ftFocus);

  // ── Renderer ───────────────────────────────────────────────────────────────
  auto ui = Renderer(layout, [&]() -> Element {
    constexpr int LEFT_W = 34;

    // ── left column ────────────────────────────────────────────────────────
    auto connBox = window(
        text(" Connection ") | bold,
        connRadio->Render());

    Elements dotRows;
    for (int i = 0; i < static_cast<int>(kActions.size()); ++i) {
      auto dot = procs[i].alive
          ? (text("● ") | color(Color::Green))
          : (text("○ ") | color(Color::GrayDark));
      dotRows.push_back(hbox({dot, text(kActions[i].name)}));
    }
    auto statusBox = window(text(" Status ") | bold, vbox(std::move(dotRows)));

    auto leftCol = vbox({connBox, statusBox}) | size(WIDTH, EQUAL, LEFT_W);

    // ── right column — actions ─────────────────────────────────────────────
    Elements actRows;
    for (int i = 0; i < static_cast<int>(kActions.size()); ++i) {
      bool sel = (i == act) && (focus == 1);

      auto tag = procs[i].alive
          ? (text(" STOP ") | color(Color::Red)   | bold)
          : (text(" RUN  ") | color(Color::Green) | bold);

      std::string hintStr = (i == MOCK_IDX)
          ? ("word: " + (mockWord.empty() ? "hello" : mockWord))
          : kActions[i].hint;

      auto row = hbox({
          tag,
          text("  "),
          text(kActions[i].name) | size(WIDTH, EQUAL, 12),
          text("  "),
          text(hintStr) | dim,
      });
      actRows.push_back(sel ? (row | inverted) : row);
    }

    auto mockRow = hbox({
        text(" Word: ") | (focus == 2 ? bold : dim),
        mockInput->Render() | size(WIDTH, EQUAL, 22),
        text("  [Tab] to type") | dim,
    });

    auto actionsBox = window(
        text(" Actions   ↑↓ navigate   Enter run·stop ") | bold,
        vbox({vbox(std::move(actRows)), separator(), mockRow}));

    // ── right column — log ─────────────────────────────────────────────────
    Elements logRows;
    {
      std::lock_guard lk(gLogMtx);
      int total   = static_cast<int>(gLog.size());
      int visible = 12;
      logScroll   = std::min(logScroll, std::max(0, total - visible));
      int end     = total - logScroll;
      int start   = std::max(0, end - visible);
      for (int i = start; i < end; ++i) {
        auto &l = gLog[i];
        // Fixed-width source column so the log is easy to scan
        std::string srcPad = l.src.substr(0, 11);
        srcPad += std::string(std::max(0, 11 - static_cast<int>(srcPad.size())), ' ');
        logRows.push_back(hbox({
            text(l.ts)     | color(Color::GrayDark),
            text(" "),
            text(srcPad)   | color(Color::Cyan),
            text(" "),
            text(l.msg),
        }));
      }
    }
    if (logRows.empty())
      logRows.push_back(text("Waiting for output...") | dim | center);

    auto logBox = window(
        text(" Output   PgUp/PgDn scroll   C clear ") | bold,
        vbox(std::move(logRows))) | flex;

    auto rightCol = vbox({actionsBox, logBox}) | flex;

    // ── bottom key-binding bar ─────────────────────────────────────────────
    auto kd = [](const std::string &k, const std::string &desc) {
      return hbox({
          text(" " + k + " ") | bold | bgcolor(Color::Blue) | color(Color::White),
          text(" " + desc + "  "),
      });
    };
    auto bar = hbox({
        kd("Tab",   "panel"),
        kd("↑↓",   "navigate"),
        kd("Enter", "run/stop"),
        kd("K",     "kill all"),
        kd("C",     "clear log"),
        kd("PgUp/Dn", "scroll"),
        kd("Q",     "quit"),
    });

    return vbox({
        text("  ★  Inertialink Smart Pen") | bold | center,
        separator(),
        hbox({leftCol, separator(), rightCol}) | flex,
        separator(),
        bar,
    });
  });

  // ── Event handler ──────────────────────────────────────────────────────────
  auto withEvents = CatchEvent(ui, [&](Event ev) -> bool {
    // Quit
    if (ev == Event::Character('q') || ev == Event::Character('Q')) {
      screen.ExitLoopClosure()();
      return true;
    }

    // Tab: cycle  conn(0) → actions(1) → mock-input(2, only when Mock selected) → conn(0)
    if (ev == Event::Tab) {
      if      (focus == 0)                    focus = 1;
      else if (focus == 1 && act == MOCK_IDX) focus = 2;
      else                                    focus = 0;
      syncFtFocus();
      return true;
    }

    // Kill all processes
    if (ev == Event::Character('k') || ev == Event::Character('K')) {
      for (auto &p : procs) if (p.alive) procStop(p);
      return true;
    }

    // Clear log (guard against stealing 'c' when typing in the word input)
    if (focus != 2 &&
        (ev == Event::Character('c') || ev == Event::Character('C'))) {
      std::lock_guard lk(gLogMtx);
      gLog.clear();
      return true;
    }

    // Actions panel navigation
    if (focus == 1) {
      if (ev == Event::ArrowUp) {
        act = std::max(0, act - 1);
        return true;
      }
      if (ev == Event::ArrowDown) {
        act = std::min(static_cast<int>(kActions.size()) - 1, act + 1);
        return true;
      }
      if (ev == Event::Return) { toggle(act); return true; }
    }

    // Log scroll
    if (ev == Event::PageUp)   { logScroll += 5;                    return true; }
    if (ev == Event::PageDown) { logScroll = std::max(0, logScroll - 5); return true; }

    return false;
  });

  // ── Background pipe-drain thread ───────────────────────────────────────────
  std::atomic<bool> bgStop{false};
  std::thread bgThread([&] {
    while (!bgStop) {
      if (procDrainAll(procs))
        screen.PostEvent(Event::Custom);
      std::this_thread::sleep_for(std::chrono::milliseconds(50));
    }
  });

  logPush("tui", "Ready — select a connection mode and press Enter on an action.");
  screen.Loop(withEvents);

  bgStop = true;
  bgThread.join();
  for (auto &p : procs) if (p.alive) procStop(p);
  return 0;
}
